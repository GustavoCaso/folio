...
- `formats/pdf/metadata.py` — `extract_metadata(pdf_path, generate_cover)`: reads title/author metadata directly from the PDF file via `pypdfium2` and, when `generate_cover` is true, also renders page 1 as a PNG thumbnail. Returns empty values on failure and never raises.
...
| `PDF_DO_FORMULA_ENRICHMENT` | `false` | Docling VLM-based formula enrichment |
...
| `HTML_INFER_FURNITURE` | `true` | Infer header/footer furniture elements |
