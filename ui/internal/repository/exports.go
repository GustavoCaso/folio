...
func (r *Repository) ListUnexportedHighlights(ctx context.Context, backendName string) ([]ExportRecord, error) {
...
func (r *Repository) ListExportsByHighlight(ctx context.Context, highlightID string) ([]ExportRecord, error) {
	rows, err := r.db.QueryContext(ctx, `
...
func (r *Repository) ListAllExports(ctx context.Context) ([]ExportRecord, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT he.id, he.highlight_id, he.backend_name, COALESCE(he.external_id, ''), he.status, COALESCE(he.error, ''), COALESCE(he.exported_at, ''),
		       h.text, COALESCE(h.note, ''), COALESCE(h.tag, ''), j.filename
		FROM highlight_exports he
