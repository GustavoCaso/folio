...
func newTestMux(t *testing.T, store *db.Store) (http.Handler, *hub.Hub) {
	...
	mux, err := handlers.Register(store, h, nil, t.TempDir(), logger, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	return mux, h
...
func newTestMuxWithParser(t *testing.T, store *db.Store, parser handlers.ParserClient) http.Handler {
	...
	mux, err := handlers.Register(store, h, parser, t.TempDir(), logger, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	return mux
...
func TestDeleteDocument_DeletesDoneJobAndFile(t *testing.T) {
	...
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	h, _ := hub.New(logger)
	mux, _ := handlers.Register(store, h, nil, dir, logger, nil, nil)
	mux.ServeHTTP(rec, req)
