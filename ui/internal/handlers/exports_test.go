...
func (r *recordingBackend) Export(_ context.Context, _ []db.HighlightWithJob) ([]export.ExportResult, error) {
	return nil, nil
}
...
func TestListExports_ShowsExportedRecord(t *testing.T) {
	...
	if err := store.MarkHighlightExported(ctx, h.ID, "readwise", "ext-123"); err != nil {
		t.Fatal(err)
	}
...
func TestListExports_ShowsFailedRecord(t *testing.T) {
	...
	if err := store.MarkHighlightExportFailed(ctx, h.ID, "readwise", "connection refused"); err != nil {
		t.Fatal(err)
	}
...
func TestDeleteHighlight_CallsBackendDelete(t *testing.T) {
	...
	if err := store.MarkHighlightExported(ctx, h.ID, "fake", "ext-999"); err != nil {
		t.Fatal(err)
	}
