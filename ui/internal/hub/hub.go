...
// Events published before any subscriber connects are buffered and replayed on Subscribe.
type Hub struct {
	mu     sync.Mutex
	subs   map[string][]chan StatusEvent
	replay map[string][]StatusEvent
	logger *slog.Logger
}
...
func New(logger *slog.Logger) (*Hub, error) {
	...
	return &Hub{
		...
		replay: make(map[string][]StatusEvent),
		logger: logger,
	}, nil
}

// Subscribe returns a channel that will receive events for the given job ID.
// Any events published before this call are replayed immediately into the channel.
func (h *Hub) Subscribe(jobID string) chan StatusEvent {
	h.mu.Lock()
	past := h.replay[jobID]
	ch := make(chan StatusEvent, len(past)+8)
	for _, e := range past {
		ch <- e
	}
	...
	n := len(h.subs[jobID])
	h.mu.Unlock()
	h.logger.Debug("subscribe", "job_id", jobID, "subscribers", n, "replayed", len(past))
	return ch
}
...
// The event is also appended to the replay buffer so late subscribers receive it.
// Slow subscribers are skipped (non-blocking send).
func (h *Hub) Publish(jobID string, event StatusEvent) {
	h.mu.Lock()
	defer h.mu.Unlock()
	h.replay[jobID] = append(h.replay[jobID], event)
